...
#define DEBUG_COUT2(message, test)                                            \
  std::cout << #message << ((test) ? " true" : " false") << std::endl;
...
void mpWindow::FillI18NString ()
{
  ...
  MESS_WILDCARD = _ (
    "Data files (*.dat)|*.dat|Csv files (csv.*)|csv.*|All files (*.*)|*.*");
...
// Legend margins
#define MARGIN_LEGEND                                                         \
  5 // The margin around the legend (the name of the function) in pixel
#define LEGEND_LINEWIDTH                                                      \
  10 // The size of the decoration in front of the legend (line or square)
...
bool DoubleToTimeStruct (double val, unsigned int timeConv,
...
mpLayer::mpLayer (mpLayerType layerType) : m_type (layerType)
{
...
mpInfoLayer::~mpInfoLayer () { DeleteAndNull (m_info_bmp); }
...
void mpInfoLayer::UpdateInfo (mpWindow &WXUNUSED (w),
...
//      wxLogMessage(_T("mpInfoLayer::Plot() screen size has changed from %d x
//      %d to %d x %d"), m_winX, m_winY, scrx, scry);
#endif
      if (m_winX != 1)
...
void mpInfoLayer::ErasePlot (wxDC &dc, mpWindow &WXUNUSED (w))
{
  if (m_info_bmp)
  {
    ...
    dc.Blit (m_oldDim.x, m_oldDim.y, m_oldDim.width, m_oldDim.height,
             &m_info_dc, 0, 0);
...
void mpInfoCoords::UpdateInfo (mpWindow &w, wxEvent &event)
{
  ...
  if (event.GetEventType () == wxEVT_MOTION)
  {
    ...
    {
      mpLayer *layer
        = w.GetClosestPlot (m_mouseX, m_mouseY, &xVal, &yVal, &isY2Axis);
      if (layer)
      {
...
wxString mpInfoCoords::GetInfoCoordsText (double xVal, double yVal,
                                          double y2Val, bool isY2Axis)
{
  wxString result;
  struct tm timestruct;
  // Format X part
  ...
  case mpX_DATETIME:
  {
    if (DoubleToTimeStruct (xVal, m_timeConv, &timestruct))
      result.Printf (_T("x = %04d-%02d-%02dT%02d:%02d:%02d"),
  ...
  {
    if (DoubleToTimeStruct (xVal, m_timeConv, &timestruct))
      result.Printf (_T("x = %04d-%02d-%02d"), timestruct.tm_year + 1900,
...
void mpInfoCoords::DoPlot (wxDC &dc, mpWindow &w)
{
  ...
  dc.DrawRectangle (m_dim);
  dc.DrawText (m_content, m_dim.x + MARGIN_COORD + offset,
               m_dim.y + MARGIN_COORD);
  if (m_series_coord)
  {
    ...
    if (ly->GetLayerType () == mpLAYER_PLOT)
    {
      if (ly->IsVisible ())
      {
        ...
        if (first)
        {
          ...
          // Since labelHeight is constant (all label layers use same legend
          // font), we can initialise height of the legend bitmap
        ...
        // Adjust the full size of the Legend and store the end (bottom or
        // right) of this legend component
        LegendDetail ld;
...
{
  // Adjust clicked point coordinates for legend bitmap's offset within plot
  // area
  wxCoord side;
  if (m_item_direction == mpVertical)
  ...
  // We only need test against right or bottom side of the rectangle (stored in
  // UpdateBitmap function).
  for (std::vector<LegendDetail>::iterator it = m_LegendDetailList.begin ();
       it != m_LegendDetailList.end (); it++)
  {
...
mpFunction::mpFunction (mpLayerType layerType /*=mpLAYER_PLOT*/,
...
void mpFunction::DrawSymbol (wxDC &dc, wxCoord x, wxCoord y)
{
  ...
  case mpsSquare:
    dc.DrawRectangle (x - m_symbolSize2, y - m_symbolSize2, m_symbolSize,
                      m_symbolSize);
    break;

  case mpsUpTriangle:
  {
    const wxPoint points[] = { wxPoint (x, y - m_symbolSize),
                               wxPoint (x + m_symbolSize, y + m_symbolSize),
                               wxPoint (x - m_symbolSize, y + m_symbolSize) };
  ...
  {
    const wxPoint points[] = { wxPoint (x - m_symbolSize, y - m_symbolSize),
                               wxPoint (x + m_symbolSize, y - m_symbolSize),
                               wxPoint (x, y + m_symbolSize) };
    dc.DrawPolygon (3, points);
    break;
  }

  case mpsCross:
    dc.DrawLine (x - m_symbolSize, y - m_symbolSize, x + m_symbolSize,
                 y + m_symbolSize);
    dc.DrawLine (x + m_symbolSize, y - m_symbolSize, x - m_symbolSize,
                 y + m_symbolSize);
    break;
...
mpLine::mpLine (double value, const wxPen &pen) : mpFunction (mpLAYER_LINE)
{
  m_value = value;
  m_subtype = mpfLine;
...
mpHorizontalLine::mpHorizontalLine (double yvalue, const wxPen &pen,
                                    bool useY2Axis)
  : mpLine (yvalue, pen)
{
...
void mpHorizontalLine::DoPlot (wxDC &dc, mpWindow &w)
{
  ...
  if (!m_drawOutsideMargins
      && ((iy > (w.GetScreenY () - w.GetMarginBottom ()))
          || (iy < w.GetMarginTop ())))
    return;
...
void mpVerticalLine::DoPlot (wxDC &dc, mpWindow &w)
{
  ...
  // Draw nothing if we are outside margins
  if (!m_drawOutsideMargins
      && ((ix > (w.GetScreenX () - w.GetMarginRight ()))
          || (ix + 1 < w.GetMarginLeft ())))
    return;
...
void mpFX::DoPlot (wxDC &dc, mpWindow &w)
{
  ...
  if (m_continuous || (m_pen.GetWidth () > 1))
  {
    if (m_continuous)
    {
      ...
      for (i = m_plotBoundaries.startPx + m_step; i < m_plotBoundaries.endPx;
           i += m_step)
      {
    ...
    {
      for (i = m_plotBoundaries.startPx; i < m_plotBoundaries.endPx;
           i += m_step)
      {
        iy = w.y2p (DoGetY (w.p2x (i)), m_UseY2Axis);
        if (m_symbol == mpsNone)
...
void mpFY::DoPlot (wxDC &dc, mpWindow &w)
{
  ...
  if (m_continuous || (m_pen.GetWidth () > 1))
  {
    if (m_continuous)
    {
      ...
      for (i = m_plotBoundaries.startPy + m_step; i < m_plotBoundaries.endPy;
           i += m_step)
      {
    ...
    {
      for (i = m_plotBoundaries.startPy; i < m_plotBoundaries.endPy;
           i += m_step)
      {
        ix = w.x2p (DoGetX (w.p2y (i, m_UseY2Axis)));
        if (m_symbol == mpsNone)
...
/**
 * Draw the newly added point at m_index, optionally drawing the
 * continuous-mode line. The bounds are already known.
 */
  ...
  if (!m_ViewAsBar)
  {
    if (m_continuous)
    {
      ...
      { // Do not draw continuous-mode line unless there is a prior point
        ...
        size_t lastPtIdx
          = m_index - 1; // we assume that m_step = 1 in this context
...
/** Add data to the internal vector. This method DOES NOT refresh the mpWindow
 * unless updatePlot = true; Do it manually by calling UpdateAll() or just
 * Fit() if we want to adjust plot BEWARE : You can not call this function if
 * mpFXYVector layer is not added to the mpWindow layers list.
 */
...
void mpProfile::DoPlot (wxDC &dc, mpWindow &w)
{
  ...
  if (!m_drawOutsideMargins)
    c0 = (c0 <= m_plotBoundaries.endPy)
           ? ((c0 >= m_plotBoundaries.startPy) ? c0 : m_plotBoundaries.startPy)
           : m_plotBoundaries.endPy;
  for (i = m_plotBoundaries.startPx + m_step; i < m_plotBoundaries.endPx;
       i += m_step)
  {
    ...
    if (!m_drawOutsideMargins)
      c1 = (c1 <= m_plotBoundaries.endPy)
             ? ((c1 >= m_plotBoundaries.startPy) ? c1
                                                 : m_plotBoundaries.startPy)
             : m_plotBoundaries.endPy;
...
mpChart::mpChart (const wxString &name) : mpFunction (mpLAYER_CHART, name)
{
...
mpBarChart::mpBarChart (const wxString &name, double width) : mpChart (name)
{
...
void mpBarChart::DoPlot (wxDC &dc, mpWindow &w)
{
  ...
  bool drawLabels
    = (labels.size () == values.size ()) && (m_labelPos != mpBAR_NONE);
  ...
  if (values.size () > 0)
  {
    for (size_t binIndex = 0; binIndex < values.size (); binIndex++)
    {
      ...
      if (drawLabels)
      {
        ...
        dc.DrawRotatedText (currentLabel, labelX, labelY,
                            m_labelAngle); // bar label
      }
    }
  }
}
...
double mpBarChart::GetMinY () { return 0.0; }

double mpBarChart::GetMaxY () { return m_max_value; }
...
mpPieChart::mpPieChart (const wxString &name, double radius) : mpChart (name)
{
  // already set in base class mpChart's ctor: m_type = mpLAYER_CHART;
  m_subtype = mpcBarChart;
  m_radius = radius;
  ...
  if (values.size () > 0)
  {
    ...
    for (size_t binIndex = 0; binIndex < values.size (); binIndex++)
    {
      ...
      if (drawLabels)
      {
        currentLabel = wxConvUTF8.cMB2WX (labels[binIndex].c_str ());
        dc.GetTextExtent (currentLabel, &labelW, &labelH);
        labelX = w.x2p (m_radius * cos (angletxt + angle / 2.0)) * scale
                 + offset + 10;
...
void mpScaleX::DrawScaleName (wxDC &dc, mpWindow &w, int origin, int labelSize)
{
  ...
  // Scale X : horizontal axis
  case mpALIGN_BORDER_BOTTOM:
    dc.DrawText (m_name, m_plotBoundaries.endPx - tx - 4,
                 origin - ty - labelSize - 8);
    break;
  case mpALIGN_BOTTOM:
  {
    if ((!m_drawOutsideMargins)
        && (w.GetMarginBottom () > (ty + labelSize + 8)))
    {
      //        dc.DrawText(m_name, (m_plotBoundaries.endPx +
      //        m_plotBoundaries.startPx - tx) / 2, orgy + labelH + 6);
      dc.DrawText (m_name, m_plotBoundaries.endPx - tx - 4,
                   origin + labelSize + 6);
    }
    else
  ...
  case mpALIGN_TOP:
  {
    ...
    {
      //        dc.DrawText(m_name, (m_plotBoundaries.endPx +
      //        m_plotBoundaries.startPx - tx) / 2, orgy - ty - labelH - 6);
      dc.DrawText (m_name, m_plotBoundaries.endPx - tx - 4,
                   origin - ty - labelSize - 8);
    }
    else
...
wxString mpScaleX::FormatValue (const wxString &fmt, double n)
{
  ...
  case mpX_DATETIME:
  {
    if (DoubleToTimeStruct (n, m_timeConv, &timestruct))
      s.Printf (fmt, timestruct.tm_year + 1900, timestruct.tm_mon + 1,
                timestruct.tm_mday, timestruct.tm_hour, timestruct.tm_min,
                timestruct.tm_sec);
  ...
  case mpX_HOURS:
  {
    ...
    wxLogMessage (_T("%02.0f Hours, %02.0f minutes, %02.0f seconds"),
                  sign * hh, mm, ss);
...
void mpScaleX::DoPlot (wxDC &dc, mpWindow &w)
{
  ...
  int labelH = 0; // Control labels height to decide where to put axis name
  ...
  for (n = n0; n < end; n += step)
  {
    ...
    if ((p >= m_plotBoundaries.startPx) && (p <= m_plotBoundaries.endPx))
    {
      ...
      {
        dc.SetPen (m_gridpen);
        dc.DrawLine (p, m_plotBoundaries.startPy + 1, p,
                     m_plotBoundaries.endPy - 1);
      }
  ...
  double labelStep
    = ceil ((maxExtent + MIN_X_AXIS_LABEL_SEPARATION) / (scaleX * step))
      * step;
...
void mpScaleY::DrawScaleName (wxDC &dc, mpWindow &w, int origin, int labelSize)
{
  ...
  case mpALIGN_LEFT:
  {
    ...
    {
      dc.DrawRotatedText (
        m_name, origin - labelSize - ty - 6,
        (m_plotBoundaries.endPy + m_plotBoundaries.startPy + tx) / 2, 90);
    }
    else
    {
  ...
  {
    if ((!m_drawOutsideMargins)
        && (w.GetMarginRight () > (ty + labelSize + 8)))
    {
      dc.DrawRotatedText (
        m_name, origin + labelSize + 6,
        (m_plotBoundaries.endPy + m_plotBoundaries.startPy + tx) / 2, 90);
    }
    else
    {
  ...
  case mpALIGN_BORDER_RIGHT:
    dc.DrawText (m_name, origin - tx - labelSize - 6,
                 m_plotBoundaries.startPy + 4);
    break;
...
void mpScaleY::DoPlot (wxDC &dc, mpWindow &w)
{
  ...
  // Draw Y axis
  dc.DrawLine (orgx + 1, m_plotBoundaries.startPy, orgx + 1,
               m_plotBoundaries.endPy);
  ...
  for (; n < end; n += step)
  {
    ...
    if ((p > m_plotBoundaries.startPy + labelHeigth)
        && (p < m_plotBoundaries.endPy - labelHeigth))
    {
      // Draw axis grids
      if (m_grids && (n != 0))
      {
        dc.SetPen (m_gridpen);
        dc.DrawLine (m_plotBoundaries.startPx + 1, p,
                     m_plotBoundaries.endPx - 1, p);
      }
...
#if defined(MATHPLOT_DO_LOGGING) && defined(MATHPLOT_LOG_SCALE)
      if (ty != labelHeigth)
        wxLogMessage (_T("mpScaleY::Plot: ty(%d) and labelHeigth(%d) differ!"),
                      ty, labelHeigth);
#endif
...
EVT_RIGHT_DCLICK (
  mpWindow::OnMouseRightDoubleClick) // Right mouse double-click
  ...
  : wxWindow (parent, id, pos, size, flag, _T("Mathplot"))
{
  ...
  wxBitmap icon[] = { wxBITMAP_PNG_FROM_DATA (Fit_24x24),
                      wxBITMAP_PNG_FROM_DATA (Zoom_in_24x24),
                      wxBITMAP_PNG_FROM_DATA (Zoom_out_24x24),
                      wxBITMAP_PNG_FROM_DATA (Center_24x24),
                      wxNullBitmap,
                      wxBITMAP_PNG_FROM_DATA (Grid_24x24),
                      wxBITMAP_PNG_FROM_DATA (Coordinates_24x24),
                      wxBITMAP_PNG_FROM_DATA (Screenshot_24x24),
                      wxBITMAP_PNG_FROM_DATA (Config_24x24),
                      wxBITMAP_PNG_FROM_DATA (Load_24x24),
                      wxBITMAP_PNG_FROM_DATA (Mouse_24x24),
  ...
  for (int i = 0; i < nbItem; i++)
  {
    ...
    {
      mymenu = new wxMenuItem (&m_popmenu, mpID_FIT + i, Popup_string[i][0],
                               Popup_string[i][1]);
      mymenu->SetBitmap (icon[i]);
      m_popmenu.Append (mymenu);
    }
    else
      m_popmenu.AppendCheckItem (mpID_LOCKASPECT, Popup_string[i][0],
                                 Popup_string[i][1]);
  }
#else
  for (int i = 0; i < nbItem; i++)
...
void mpWindow::OnMouseLeftDown (wxMouseEvent &event)
{
...
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage (_T("mpWindow::OnMouseLeftDown() X = %d , Y = %d"),
  ...
  {
    wxLogMessage (_T("mpWindow::OnMouseLeftDown() started moving layer %p"),
                  m_movingInfoLayer);
  }
#endif
  if (m_InInfoLegend)
...
// Mouse handler, for detecting when the user drag with the right button or
// just "clicks" for the menu JLB
...
void mpWindow::OnMouseMove (wxMouseEvent &event)
{
...
#ifdef MATHPLOT_DO_LOGGING
    wxLogMessage (
      _T("[mpWindow::OnMouseMove] Ax:%i Ay:%i m_posX:%f m_posY:%f"), Axy.x,
      Axy.y, m_posX, m_posY);
#endif
  }
  else
  {
    ...
    if (event.m_leftDown)
    {
      ...
      if (m_movingInfoLayer == NULL)
      {
        ...
        if (m_zoom_bmp)
        {
          ...
          dc.Blit (m_zoom_oldDim.x, m_zoom_oldDim.y, m_zoom_oldDim.width,
                   m_zoom_oldDim.height, &m_coord_dc, 0, 0);
          m_coord_dc.SelectObject (wxNullBitmap);
          DeleteAndNull (m_zoom_bmp);
        }
        ...
        // Second : store new bitmap
        m_zoom_dim
          = wxRect (m_mouseLClick, wxSize (moveVector.x, moveVector.y));
        if ((m_zoom_dim.width != 0) && (m_zoom_dim.height != 0))
        {
          ...
          wxMemoryDC m_coord_dc (&dc);
          m_coord_dc.SelectObject (*m_zoom_bmp);
          m_coord_dc.Blit (0, 0, m_zoom_dim.width, m_zoom_dim.height, &dc,
                           m_zoom_dim.x, m_zoom_dim.y);
          m_coord_dc.SelectObject (wxNullBitmap);
    ...
    else
    {
      ...
      {
        if ((m_InfoCoords->GetDrawOutsideMargins ())
            || (m_PlotArea.Contains (eventPoint)))
        {
...
void mpWindow::Fit (const mpFloatRect &rect, wxCoord *printSizeX,
                    wxCoord *printSizeY)
{ // JL
  ...
  {
#ifdef MATHPLOT_DO_LOGGING
    wxLogMessage (_T("mpWindow::Fit()(lock) m_scaleX=%f,m_scaleY=%f"),
                  m_scaleX, m_scaleY);
#endif
  ...
  m_posX = (rect.Xmin + rect.Xmax) / 2
           - (m_plotWidth / 2 + m_margin.left) / m_scaleX;
  m_posY = (rect.Ymin + rect.Ymax) / 2
           + (m_plotHeight / 2 + m_margin.top) / m_scaleY;
  m_posY2 = (rect.Y2min + rect.Y2max) / 2
            + (m_plotHeight / 2 + m_margin.top) / m_scaleY2;

#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage (_T("mpWindow::Fit() m_desired.Xmin=%f m_desired.Xmax=%f  ")
                _T("m_desired.Ymin=%f m_desired.Ymax=%f"),
                m_desired.Xmin, m_desired.Xmax, m_desired.Ymin,
                m_desired.Ymax);
  wxLogMessage (_T("mpWindow::Fit() m_scaleX = %f , m_scrX = %d,m_scrY=%d, ")
                _T("Ax=%f, Ay=%f, m_posX=%f, m_posY=%f"),
                m_scaleX, m_scrX, m_scrY, Ax, Ay, m_posX, m_posY);
#endif

  // It is VERY IMPORTANT to DO NOT call Refresh if we are drawing to the
  // printer!! Otherwise, the DC dimensions will be those of the window instead
...
{
  if (m_desired.IsNotSet ())
    return; // nothing to do until useful info in m_desired
...
void mpWindow::DoZoomInXCalc (const int staticXpixel)
{
  ...
  CheckAndReportDesiredBoundsChanges ();
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage (_T("mpWindow::DoZoomInXCalc() prior X coord: (%f), new X ")
                _T("coord: (%f) SHOULD BE EQUAL!!"),
...
void mpWindow::DoZoomInYCalc (const int staticYpixel)
{
  ...
  CheckAndReportDesiredBoundsChanges ();
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage (_T("mpWindow::DoZoomInYCalc() prior Y coord: (%f), new Y ")
                _T("coord: (%f) SHOULD BE EQUAL!!"),
...
void mpWindow::DoZoomOutXCalc (const int staticXpixel)
{
  ...
  CheckAndReportDesiredBoundsChanges ();
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage (_T("mpWindow::DoZoomOutXCalc() prior X coord: (%f), new X ")
                _T("coord: (%f) SHOULD BE EQUAL!!"),
                staticX, p2x (staticXpixel));
#endif
}
...
void mpWindow::DoZoomOutYCalc (const int staticYpixel)
{
  ...
  CheckAndReportDesiredBoundsChanges ();
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage (_T("mpWindow::DoZoomOutYCalc() prior Y coord: (%f), new Y ")
                _T("coord: (%f) SHOULD BE EQUAL!!"),
                staticY, p2y (staticYpixel));
#endif
}
...
void mpWindow::Zoom (bool zoomIn, const wxPoint &centerPoint)
{
  ...
  double zoomFactor
    = zoomIn ? m_zoomIncrementalFactor : 1.0 / m_zoomIncrementalFactor;
  m_scaleX *= zoomFactor;
  m_scaleY *= zoomFactor;
...
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage (_T("mpWindow::Zoom() prior coords: (%f,%f), new coords: ")
                _T("(%f,%f) SHOULD BE EQUAL!!"),
...
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage (_T("Zoom: (%f,%f)-(%f,%f)"), zoom.Xmin, zoom.Ymin, zoom.Xmax,
                zoom.Ymax);
#endif
...
{
  mpInfoCoords *coords
    = (mpInfoCoords *)GetLayerByClassName (_T("mpInfoCoords"));
  if (coords)
  {
...
void mpWindow::OnCenter (wxCommandEvent &WXUNUSED (event))
{
  ...
  SetPos (p2x (m_clickedX - centerX - m_margin.left),
          p2y (m_clickedY - centerY - m_margin.top),
...
void mpWindow::OnSize (wxSizeEvent &WXUNUSED (event))
{
  ...
  Fit (m_desired);
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage (_T("mpWindow::OnSize() m_scrX = %d, m_scrY = %d"), m_scrX,
                m_scrY);
#endif // MATHPLOT_DO_LOGGING
}
...
bool mpWindow::DelLayer (mpLayer *layer, bool alsoDeleteObject,
                         bool refreshDisplay)
{
  for (mpLayerList::iterator it = m_layers.begin (); it != m_layers.end ();
       it++)
  {
    if (*it == layer)
    {
      ...
      if (m_OnDeleteLayer != NULL)
        m_OnDeleteLayer (this, (*it)->GetClassInfo ()->GetClassName (),
                         cancel);
      if (!cancel)
      {
        ...
        // WARNING: 'erase' invalidates 'it' and all m_layers iterators in
        // existence
        m_layers.erase (it);
...
void mpWindow::DelAllPlot (bool alsoDeleteObject, mpFunctionType func,
                           bool refreshDisplay)
{
  int function;
  ...
  do
  {
    if ((*it)->IsLayerType (mpLAYER_PLOT, &function)
        && ((func == mpfAllType) || (function == func)))
    {
      DelLayer ((mpLayer *)(*it), alsoDeleteObject,
                false); // may invalidate all extant m_layers iterators
      it = m_layers
             .begin (); // ...so reset iterator to begin of m_layers vector
    }
    else
      it++;
...
void mpWindow::OnPaint (wxPaintEvent &WXUNUSED (event))
{
...
#ifdef MATHPLOT_DO_LOGGING
  {
    int px = 0, py = 0;
    //    GetViewStart( &px, &py );
    wxLogMessage (_T("[mpWindow::OnPaint] vis.area:%ix%i px=%i py=%i"), m_scrX,
                  m_scrY, px, py);
  }
#endif
  ...
  // We redraw the cross if necessary. We pass the mouse position if we do a
  // pan operation.
  if (m_magnetize)
...
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage (_T("[mpWindow::UpdateBBox] Bounding box enter: Xmin = %f, ")
                _T("Xmax = %f, Ymin = %f, YMax = %f"),
                m_bound.Xmin, m_bound.Xmax, m_bound.Ymin, m_bound.Ymax);
#endif // MATHPLOT_DO_LOGGING
...
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage (_T("[mpWindow::UpdateBBox] Bounding box exit: Xmin = %f, ")
                _T("Xmax = %f, Ymin = %f, YMax = %f"),
                m_bound.Xmin, m_bound.Xmax, m_bound.Ymin, m_bound.Ymax);
#endif // MATHPLOT_DO_LOGGING
  return first == false;
  ...
  if (UpdateBBox ())
  {
    if (m_enableScrollBars)
    {
      ...
      // Do x scroll bar
      {
        ...
        double maxX
          = (m_desired.Xmax > m_bound.Xmax) ? m_desired.Xmax : m_bound.Xmax;
        double minX
          = (m_desired.Xmin < m_bound.Xmin) ? m_desired.Xmin : m_bound.Xmin;
        ...
        SetScrollbar (wxHORIZONTAL, thumbX,
                      cx - (m_margin.right + m_margin.left), sizeX);
      }
      // Do y scroll bar
      {
        // Convert margin sizes from pixels to coordinates
        ...
        // Calculate the range in coords that we want to scroll over
        double maxY
          = (m_desired.Ymax > m_bound.Ymax) ? m_desired.Ymax : m_bound.Ymax;
        if ((m_posY - topMargin) > maxY)
          maxY = m_posY - topMargin;
        double minY
          = (m_desired.Ymin < m_bound.Ymin) ? m_desired.Ymin : m_bound.Ymin;
        // Calculate scroll bar size and thumb position
        int sizeY = (int)((maxY - minY) * m_scaleY);
        int thumbY = (int)((maxY - (m_posY - topMargin)) * m_scaleY);
        SetScrollbar (wxVERTICAL, thumbY,
                      cy - (m_margin.top + m_margin.bottom), sizeY);
      }
    }
  }
...
void mpWindow::DoScrollCalc (const int position, const int orientation)
{
  if (orientation == wxVERTICAL)
  {
    ...
    double maxY
      = m_desired.Ymax > m_bound.Ymax ? m_desired.Ymax : m_bound.Ymax;
    double maxY2
      = m_desired.Y2max > m_bound.Y2max ? m_desired.Y2max : m_bound.Y2max;
    // Set new position
    SetPosY ((maxY - (position / m_scaleY)) + topMargin,
             (maxY2 - (position / m_scaleY2)) + topMargin2);
  }
  else
  {
    // X Axis
 ...
 * - CountLayersFXYPlot() : return the number of layer who are FXYVector
 * function plot
 * - GetLayer(int position) : return the layer at position
 * - GetLayerPlot(int position, mpFunctionType func) : return the layer at
 * position if his function type is func
 */
...
mpLayer *mpWindow::GetLayerAxis (int position, mpScaleType scale)
{
  ...
  for (mpLayerList::iterator it = m_layers.begin (); it != m_layers.end ();
       it++)
  {
    if ((*it)->IsLayerType (mpLAYER_AXIS, &thescale)
        && ((scale == mpsAllType) || (thescale == scale)))
    {
      if (++layerNo == position)
        return *it;
    }
...
mpFXYVector *mpWindow::GetXYSeries (unsigned int n, const wxString &name,
                                    bool create)
{
...
mpLayer *mpWindow::GetClosestPlot (wxCoord ix, wxCoord iy, double *xnear,
...
#define NEAR_AREA 8
  ...
  for (mpLayerList::iterator it = m_layers.begin (); it != m_layers.end ();
       it++)
  {
    if ((*it)->IsTractable () && (*it)->IsVisible ()
        && (*it)->IsLayerType (mpLAYER_PLOT, &function))
    {
      switch (function)
      {
      case mpfFX:
        ...
        else
        {
          ...
          while (fxy->DoGetNextXY (&xx, &yy))
          {
            if ((abs (this->x2p (xx) - ix) < NEAR_AREA)
                && (abs (this->y2p (yy, fxy->GetY2Axis ()) - iy) < NEAR_AREA))
            {
              *xnear = xx;
              *ynear = yy;
              *isY2Axis = fxy->GetY2Axis ();
...
{
  mpInfoLegend *legend
    = (mpInfoLegend *)GetLayerByClassName (_T("mpInfoLegend"));
  if (legend)
...
mpInfoLayer *mpWindow::IsInsideInfoLayer (const wxPoint &point)
{
  ...
  {
#ifdef MATHPLOT_DO_LOGGING
    wxLogMessage (_T("mpWindow::IsInsideInfoLayer() examinining layer = %p"),
                  (*it));
#endif // MATHPLOT_DO_LOGGING
...
void mpWindow::SetMargins (int top, int right, int bottom, int left)
{
  ...
  m_PlotArea
    = wxRect (m_margin.left - EXTRA_MARGIN, m_margin.top - EXTRA_MARGIN,
                               ...
                               const wxColour &axesColour)
{
       ...
       it++)
  {
    ...
    wxPen pen = (*it)->GetPen (); // Get the old pen to modify only colour, not
                                  // style or width
 ...
 */
mpText::mpText (const wxString &name, int offsetx, int offsety) : mpText (name)
{
...
mpPrintout::mpPrintout (mpWindow *drawWindow, const wxString &title,
                        int factor)
  : wxPrintout (title)
{
  drawn = false;
...
bool mpPrintout::OnPrintPage (int page)
{
  ...
  if ((trgDc) && (page == 1))
  {
    ...
    plotWindow->BitmapScreenshot (
      wxSize (m_prnXw / stretch_factor, m_prnYw / stretch_factor));
    ...
    if (stretch_factor == 1)
      ...
      trgDc->StretchBlit (marginX, marginY, m_prnXw, m_prnYw, &bitmap_dc, 0, 0,
...
void mpMovableObject::ShapeUpdated ()
{
  ...
  {
    wxLogError (_T("[mpMovableObject::ShapeUpdated] Error, m_shape_xs and ")
                _T("m_shape_ys have different lengths!"));
  }
  else
  {
    double ccos, csin;
...
void mpCovarianceEllipse::RecalculateShape ()
{
  ...
  {
    wxLogError (
      _T("[mpCovarianceEllipse] Error: quantiles must be non-negative"));
    return;
  }
  if (m_cov_00 < 0)
  {
    wxLogError (
      _T("[mpCovarianceEllipse] Error: cov(0,0) must be non-negative"));
    return;
  }
  if (m_cov_11 < 0)
  {
    wxLogError (
      _T("[mpCovarianceEllipse] Error: cov(1,1) must be non-negative"));
    return;
  }
  ...
  {
    wxLogError (
      _T("[mpCovarianceEllipse] Error: cov is not positive definite"));
    return;
  }
                           ...
                           bool closedShape)
{
  ...
  {
    wxLogError (_T("[mpPolygon] Error: points_xs and points_ys must have the ")
                _T("same number of elements"));
  }
  else
  {
...
void mpBitmapLayer::DoPlot (wxDC &dc, mpWindow &w)
{
  ...
  /*  1st: We compute (x0,y0)-(x1,y1), the pixel coordinates of the real outer
   limits of the image rectangle within the (screen) mpWindow. Note that these
   coordinates might fall well far away from the real view limits when the user
   zoom in.

   2nd: We compute (dx0,dy0)-(dx1,dy1), the pixel coordinates the rectangle
   that will be actually drawn into the mpWindow, i.e. the clipped real
   rectangle that avoids the non-visible parts. (offset_x,offset_y) are the
   pixel coordinates that correspond to the window point (dx0,dy0) within the
   image "m_bitmap", and (b_width,b_height) is the size of the bitmap patch
   that will be drawn.
  ...
  // 2nd step -------------------------------
  // Precompute the size of the actual bitmap pixel on the screen (e.g. will be
  // >1 if zoomed in)
  ...
  // and the size in pixel of the area to be actually drawn from the internally
  // stored bitmap:
...
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage (
    _T("[mpBitmapLayer::Plot] screenPixel: x=%f y=%f  d_width=%ix%i"),
...
void mpMagnet::DrawCross (wxClientDC &dc) const
{
  ...
  dc.DrawLine (m_mousePosition.x, m_plot_size.y, m_mousePosition.x,
               m_plot_size.height);
  dc.DrawLine (m_plot_size.x, m_mousePosition.y, m_plot_size.width,
               m_mousePosition.y);
