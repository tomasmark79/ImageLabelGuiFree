...
/** @mainpage wxMathPlot
 ...
 vertical rulers: mpScaleX and mpScaleY. For convenient function plotting a
 series of classes derived from mpLayer are provided, like mpFX, mpProfile,
 mpLegend and so on. These base classes already come with plot code, user's own
 functions can be implemented by overriding just one member for retrieving a
 function value.
 ...
 printers or exported in bitmap formats like PNG, BMP or JPEG.
 ...
 <cdron77@users.sourceforge.net>.<br> Authors can be contacted via the
 wxMathPlot's homepage at https://sourceforge.net/projects/wxmathplot<br>
 Contributors:<br>
...
/** When setting x labels in date/time format, convert input time to local
 * time. */
#define mpX_LOCALTIME 0x10
/** When setting x labels in date/time format, convert input time to UTC time
...
#define EXTRA_MARGIN 8
  ...
  struct mpFloatRect
  {
    ...
    mpFloatRect ()
      : Xmin (0.0), Xmax (0.0), Ymin (0.0), Ymax (0.0), Y2min (0.0),
        Y2max (0.0)
    {
    }
...
/// Equal operator
  ...
  && (__cplusplus                                                             \
    ...
    // NOTE: should be unnecessary as we are looking for any changes; normally
    // this will be an exact match or a real change...
    bool operator== (const mpFloatRect &rect) const
    {
      auto Same
        = [this, rect] (double a, double b) { return fabs (a - b) < EPSILON; };
  ...
  /** Show legend items with small square with the same color of referred
   * mpLayer  */
  ...
  {
    mpVertical,  //!< each visible plot is described on its own line, one above
                 //!< the other
    mpHorizontal //!< legend components follow each other horizontally on a
                 //!< single line
  } mpLegendDirection;
   ...
   * Then draw axis, custom layer, function, info (info, coord, legend, ...)
   * and finally text (text, title)
   */
  ...
  public:
     ...
     */
    void SetWindow (mpWindow &w) { m_win = &w; }
    ...
    /**
     * Return the bounding box, ie GetMinX(), GetMaxX(), GetMinY() and
     * GetMaxY() of this mpLayer Here we don't use extra Y2 values
     */
    virtual void GetBBox (mpFloatRect *m_bound);

    /** Get layer type: a Layer can be of different types: plot, lines, axis,
     info boxes, etc, this method returns the right value.
    ...
    int GetLayerSubType () const { return m_subtype; }
    ...
    /** Plot given view of layer to the given device context.
     ...
     mpWindow passed in \a w. Note that the public methods of mpWindow: x2p,y2p
     and p2x,p2y are already provided which transform layer coordinates to DC
     pixel coordinates, and <b>user code should rely on them</b> for
     portability and future changes to be applied transparently, instead of
     implementing the following formulas manually.

     The passed device context \a dc has its coordinate origin set to the
     top-left corner of the visible area (the default). The coordinate
     orientation is as shown in the following picture: <pre> (wxDC origin 0,0)
     x-------------> ascending X ----------------+
     |                                           |
     |                                           |
     V ascending Y                               |
     ...
     the mpWindow visible area.
     </pre>
     Note that Y ascends in downward direction, whereas the usual vertical
     orientation for mathematical plots is vice versa. Thus Y-orientation will
     be swapped usually, when transforming between wxDC and mpLayer
     coordinates. This change of coordinates is taken into account in the
     methods p2x, p2y, x2p, y2p.
     ...
     guaranteed to be not 0 layer_Y = mpWindow::GetPosY() - (dc_Y /
     mpWindow::GetScaleY()) // swapping Y-orientation
     @endcode
    ...
    /** Get font foreground colour set for this layer.
     @return colour
     */
    const wxColour &GetFontColour () const { return m_fontcolour; }
    ...
    inline bool GetShowName () const { return m_showName; }

    /** Set Draw mode: inside or outside margins. Default is outside (TRUE),
     which allows the layer to draw up to the mpWindow border.
    ...
    bool GetDrawOutsideMargins () const { return m_drawOutsideMargins; }
    ...
    /** Sets layer visibility.
     @param show visibility bool. */
    virtual void SetVisible (bool show) { m_visible = show; }

    /** Checks whether the layer is tractable or not.
     @return \a true if visible */
    inline bool IsTractable () const { return m_tractable; }

    /** Sets layer tractability.
     @param track tractability bool. */
    virtual void SetTractable (bool track) { m_tractable = track; }
    ...
    /** Get CanDelete for plot.
     @return CanDelete*/
    bool GetCanDelete (void) const { return m_CanDelete; }

    /** Get the ZIndex of the plot.
     @return m_ZIndex*/
    mpLayerZOrder GetZIndex (void) const { return m_ZIndex; }
  ...
  protected:
    ...
    wxPen m_pen; //!< Layer's pen. Default Colour = Black, width = 1, style =
                 //!< wxPENSTYLE_SOLID
    ...
    bool m_drawOutsideMargins; //!< Select if the layer should draw only inside
                               //!< margins or over all DC. Default : false
    ...
    mpRect m_plotBoundaries; //!< The boundaries for plotting curve calculated
                             //!< by mpWindow
  ...
  public:
    ...
    /** Updates the content of the info box. Should be overridden by derived
     classes. Update may behave in different ways according to the type of
     event which called it.
    ...
    wxPoint GetPosition () const { return m_dim.GetPosition (); }

    /** Returns the size of the box (in pixels)
     @return The rectangle size */
    wxSize GetSize () const { return m_dim.GetSize (); }
     ...
     */
    void SetLocation (mpLocation location) { m_location = location; }

    /** Return the location of the mpInfoLayer box
     @return location */
    mpLocation GetLocation () const { return m_location; }

  protected:
    wxRect m_dim;    //!< The bounding rectangle of the mpInfoLayer box (may be
                     //!< resized dynamically by the Plot method).
    ...
    mpLocation m_location; //!< Location of the box in the margin. Default
                           //!< mpMarginNone = use coordinates
  ...
  /** @class mpInfoCoords
   ...
   units. When mouse is moved over the mpWindow with an active mpInfoCoords
   layer, the mouse coordinates (in mpWindow units, not pixels) are
   continuously reported inside the layer box. */
  ...
  public:
     ...
     */
    void SetSeriesCoord (bool show) { m_series_coord = show; }

    /** Return if we show the series coordinates
     @return bool */
    bool IsSeriesCoord () const { return m_series_coord; }

    /// Get string describing mouse position. Override in your derived class to
    /// customize mpInfoCoords display.
    /// @param xVal,yVal,y2Val Position of mouse in user coordinates
    /// @param isY2Axis Best guess as to whether closest point to mouse is a
    /// plot layer using Y2 axis
    virtual wxString GetInfoCoordsText (double xVal, double yVal, double y2Val,
                                        bool isY2Axis = false);

    /** Pen series for tractable
     */
    void SetPenSeries (const wxPen &pen) { m_penSeries = pen; }
  ...
  public:
    ...
    mpInfoLegend (wxRect rect, const wxBrush &brush = *wxWHITE_BRUSH,
                  mpLocation location = mpMarginNone);

    /**  Default destructor */
    ~mpInfoLegend () {}

    /** Set item mode (the element on the left of text representing the plot
     * line may be line or square).
    ...
    mpLegendStyle GetItemMode () const { return m_item_mode; }
    ...
    mpLegendDirection GetItemDirection () const { return m_item_direction; }

    void SetNeedUpdate () { m_needs_update = true; }
  ...
  private:
    ...
    struct LegendDetail
    {
      ...
      wxCoord legendEnd;     //!< right side (if horizontal) or bottom side (if
                         //!< vertical) of the area occupied by the function
                         //!< name and decoration
    };
    std::vector<LegendDetail>
      m_LegendDetailList; //!< list (well, vector) of details for each
                          //!< individual plot's legend component
    bool m_needs_update;  //!< Do we need to redraw the legend bitmap? Set when
                         //!< a plot function changes (name, visibility, add or
                         //!< remove)
    /**
  ...
  public:
    /** Full constructor.
     */
    mpFunction (mpLayerType layerType = mpLAYER_PLOT,
                const wxString &name = wxEmptyString, bool useY2Axis = false);

    /** Set the 'continuity' property of the layer. True: draws a continuous
     * line. False: draws separate points (default).
     * @sa GetContinuity
     */
    ...
    void SetSymbol (mpSymbol symbol) { m_symbol = symbol; }

    /** Get symbol.
     @return symbol */
    mpSymbol GetSymbol () const { return m_symbol; }
     ...
     */
    void SetY2Axis (bool _useY2) { m_UseY2Axis = _useY2; }

    /** Is second Y axis selected for this layer?
     @return UseY2Axis
     */
    bool GetY2Axis () const { return m_UseY2Axis; }

  protected:
    bool m_continuous; //!< Specify if the layer will be plotted as a
                       //!< continuous line or a set of points. Default false
    mpSymbol
      m_symbol; //!< A symbol for the plot in place of point. Default mpNone
  ...
  public:
    ...
    // We don't want to include line (horizontal or vertical) in BBox
    // computation
     ...
     */
    bool IsHorizontal (void) const { return m_IsHorizontal; }
  ...
  {
  public:
    mpHorizontalLine (double yvalue, const wxPen &pen = *wxGREEN_PEN,
                      bool useY2Axis = false);

    /** Set y
     @param yvalue
     */
    void SetYValue (const double yvalue) { SetValue (yvalue); }
  ...
  public:
    ...
    /** Set x
     @param xvalue
     */
    void SetXValue (const double xvalue) { SetValue (xvalue); }
  ...
  /** Abstract base class providing plot and labeling functionality for
   functions F:X->Y. Override mpFX::GetY to implement a function. Override
   mpFX::GetMinY and mpFX::GetMaxY to provide min and max Y range Optionally
   implement a constructor and pass a name (label) and a label alignment to the
   constructor mpFX::mpFX. If the layer name is empty, no label will be
   plotted.
   */
  ...
  /** Abstract base class providing plot and labeling functionality for
   functions F:Y->X. Override mpFY::GetX to implement a function. Override
   mpFY::GetMinX and mpFY::GetMaxX to provide min and max X range Optionally
   implement a constructor and pass a name (label) and a label alignment to the
   constructor mpFY::mpFY. If the layer name is empty, no label will be
   plotted.
   */
  ...
  /** Abstract base class providing plot and labeling functionality for a locus
   plot F:N->X,Y. Locus argument N is assumed to be in range 0 .. MAX_N, and
   implicitly derived by enumerating all locus values. Override mpFXY::Rewind
   and mpFXY::GetNextXY to implement a locus. Override mpFXY::GetMinX and
   mpFXY::GetMaxX to provide min and max X range Override mpFXY::GetMinY and
   mpFXY::GetMaxY to provide min and max Y range Optionally implement a
   constructor and pass a name (label) and a label alignment to the constructor
  ...
  public:
    ...
    mpFXY (const wxString &name = wxEmptyString, int flags = mpALIGN_NE,
           bool viewAsBar = false, bool useY2Axis = false);
     ...
     */
    int GetBarWidth (void) const { return m_BarWidth; }

    /**
     * return true if XY series is plotted with bar
     */
    bool ViewAsBar (void) const { return m_ViewAsBar; }
  ...
  /** A class providing graphs functionality for a 2D plot (either continuous
   or a set of points), from vectors of data. This class can be used directly,
   the user does not need to derive any new class. Simply pass the data as two
   vectors with the same length containing the X and Y coordinates to the
   method SetData.
  ...
  public:
     ...
     * @sa Clear
     */
    void SetData (const std::vector<double> &xs,
     ...
     */
    virtual int GetSize () { return m_xs.size (); }

    /** Add data to the internal vector. This method DOES NOT refresh the
     * mpWindow unless updatePlot = true and the added point is in bound; do it
     * manually by calling UpdateAll() or just Fit() if we want to adjust plot
     * @param x
     * @param y
     * @param updatePlot. boolean, set true to update plot. This speed the
     * rendering because just new point is drawing.
     ...
     * Note :
     * this does not modify the size of m_xs and m_ys, this is not a resize.
     * Call Clear() to resize to 0. if capacity is already superior at reserve,
     * do nothing
     */
     ...
     */
    int GetReserve () const { return m_reserveXY; }
  ...
  protected:
     ...
     */
    virtual double GetMinX () { return m_minX; }

    /** Returns the actual minimum Y data (loaded in SetData).
     */
    virtual double GetMinY () { return m_minY; }
  ...
  /** Abstract base class providing plot and labeling functionality for
   functions F:Y->X. Override mpProfile::GetX to implement a function. This
   class is similar to mpFY, but the Plot method is different. The plot is in
   fact represented by lines instead of points, which gives best rendering of
   rapidly-varying functions, and in general, data which are not so close one
   to another. Optionally implement a constructor and pass a name (label) and a
   label alignment to the constructor mpProfile::mpProfile. If the layer name
   is empty, no label will be plotted.
   */
  ...
  public:
    ...
    virtual bool HasBBox () { return (values.size () > 0); }
...
#define mpBAR_TOP 4    //!< Labels over the bar
  ...
  public:
    ...
    void SetColumnWidth (const double colWidth) { m_width = colWidth; }
  ...
  public:
    ...
    /** Get inclusive left border of bounding box.
     @return Value
     */
    virtual double GetMinX () { return -m_radius; }

    /** Get inclusive right border of bounding box.
     @return Value
     */
    virtual double GetMaxX () { return m_radius; }

    /** Get inclusive bottom border of bounding box.
     @return Value
     */
    virtual double GetMinY () { return -m_radius; }

    /** Get inclusive top border of bounding box.
     @return Value
     */
    virtual double GetMaxY () { return m_radius; }
  ...
  public:
    /** Full constructor.
     ...
     @param grids Show grid or not. Give false (default) for not drawing the
     grid. */
    ...
    void ShowTicks (bool ticks) { m_ticks = ticks; }

    /** Get axis ticks
     @return true if ticks are shown */
    bool GetShowTicks () const { return m_ticks; }

    /** Set axis grids
     @param grids TRUE to plot axis grids. */
    void ShowGrids (bool grids) { m_grids = grids; }

    /** Get axis grids
     @return true if grids are shown */
    bool GetShowGrids () const { return m_grids; }
    ...
    /** Set grid pen
     @param pen Pen, will be copied to internal class member
     */
    void SetGridPen (const wxPen &pen) { m_gridpen = pen; }

    /** Get pen set for this axis.
     @return Pen
     */
    const wxPen &GetGridPen () const { return m_gridpen; }
     ...
     */
    bool GetAuto () const { return m_auto; }

    void SetMinScale (double min) { m_min = min; }

    double GetMinScale () const { return m_min; }

    void SetMaxScale (double max) { m_max = max; }

    double GetMaxScale () const { return m_max; }

    virtual bool IsLogAxis () = 0;
    virtual void SetLogAxis (bool log) = 0;

  protected:
    wxPen m_gridpen; //!< Grid's pen. Default Colour = LIGHT_GREY, width = 1,
    ...
    virtual void DrawScaleName (wxDC &dc, mpWindow &w, int origin,
                                int labelSize)
      = 0;
...
#define mpX_NONE 0x06
  ...
  public:
    /** Full constructor.
     ...
     @param grids Show grid or not. Give false (default) for not drawing the
     grid.
     @param type mpX_NORMAL for normal labels, mpX_TIME for time axis in hours,
     minutes, seconds. */
    mpScaleX (const wxString &name = _T("X"), int flags = mpALIGN_CENTERX,
              bool grids = false, unsigned int type = mpX_NORMAL)
    ...
    /** Get X axis label view mode.
     ...
     minutes, seconds. */
    unsigned int GetLabelMode () const { return m_labelType; }
  ...
  protected:
    unsigned int
      m_labelType; //!< Select labels mode: mpX_NORMAL for normal labels,
                   //!< mpX_TIME for time axis in hours, minutes, seconds
    unsigned int m_timeConv; //!< Selects if time has to be converted to local
                             //!< time or not.
  ...
  /** Plot layer implementing a y-scale ruler.
   ...
   plotted at the top-right hand of the ruler. The scale numbering
   automatically adjusts to view and zoom factor.
   */
  ...
  public:
    ...
    mpScaleY (const wxString &name = _T("Y"), int flags = mpALIGN_CENTERY,
              bool grids = false, bool Y2Axis = false)
      : mpScale (name, flags, grids)
    {
      m_subtype = mpsScaleY;
      m_isY2Axis = Y2Axis;
    ...
    virtual bool IsY2Axis () { return m_isY2Axis; }
...
#define mpMOUSEMODE_ZOOMBOX 1
   ...
   * Use like this :
   *  your_plot->SetOnDeleteLayer([this](void *Sender, const wxString
   &classname, bool &cancel) {  your_event_function(Sender, classname,
   cancel);});
   */
  typedef std::function<void (void *Sender, const wxString &classname,
                              bool &cancel)>
    mpOnDeleteLayer;
   ...
   * Use like this :
   *  your_plot->SetOnUserMouseAction([this](void *Sender, wxMouseEvent &event,
   bool &cancel) {  your_event_function(Sender, event, cancel);});
   */
  typedef std::function<void (void *Sender, wxMouseEvent &event, bool &cancel)>
    mpOnUserMouseAction;
  ...
  public:
    ...
    void UpdateBox (const wxRect &size)
    {
      ...
      m_plot_size = wxRect (size.GetLeft (), size.GetTop (),
                            size.GetWidth () + size.GetLeft (),
                            size.GetHeight () + size.GetTop ());
    }
    ...
    void SetRightClick (void) { m_rightClick = true; }

  private:
    wxRect m_domain; //!< The area delimited by axis (m_margin.left,
                     //!< m_margin.top, m_plotWidth, m_plotHeight)
    wxRect
      m_plot_size; //!< The coordinates for the cross (xmin, xmax), (ymin,ymax)
    wxPoint m_mousePosition; //!< The last position of the mouse
    bool m_IsDrawn;          //!< Is that the cross is drawn ?
    bool
      m_IsWasDrawn; //!< Is that the cross was drawn before the OnPaint event ?
  ...
  /** Canvas for plotting mpLayer implementations.
   ...
   view. The context menu can be retrieved with mpWindow::GetPopupMenu, e.g.
   for extending it externally.
  ...
  public:
    ...
    /** Add a plot layer to the canvas.
     ...
     of mpWindow, i.e. it will be delete'd on mpWindow destruction
   ...
   after removing the layer.
     ...
     N.B. If alsoDeleteObject is false, only the layer pointer in the mpWindow
   is removed, the layer object still exists. WARNING: Invalidates any extant
   m_layers iterators!
     */
    bool DelLayer (mpLayer *layer, bool alsoDeleteObject,
    ...
    /** Remove all plot layers.
     ...
     after removing the layers.
     */
    void DelAllPlot (bool alsoDeleteObject, mpFunctionType func = mpfAllType,
                     bool refreshDisplay = true);
    ...
    /*! Get the layer plot in list position indicated.
     N.B. You <i>must</i> know the index of the layer plot inside the list of
     plot!
     ...
     */
    mpFXYVector *GetXYSeries (unsigned int n,
     ...
     */
    mpLayer *GetClosestPlot (wxCoord ix, wxCoord iy, double *xnear,
                             double *ynear, bool *isY2Axis);
    ...
    mpFloatRect Get_Bound (void) const { return m_bound; }
    ...
    void SetScreen (const int scrX, const int scrY)
    {
      ...
      m_PlotArea = wxRect (
        m_margin.left - EXTRA_MARGIN, m_margin.top - EXTRA_MARGIN,
    ...
    /** Get current view's X dimension in device context units.
     ...
     @return X dimension.
     */
    int GetScreenX (void) const { return m_scrX; }
    ...
    /** Get current view's Y dimension in device context units.
     ...
     @return Y dimension.
     */
    int GetScreenY (void) const { return m_scrY; }
    ...
    /** Converts mpWindow (screen) pixel coordinates into graph (floating
     * point) coordinates, using current mpWindow position and scale.
     * @sa p2y,x2p,y2p */
    ...
    /** Converts graph (floating point) coordinates into mpWindow (screen)
     * pixel coordinates, using current mpWindow position and scale.
     * @sa p2x,p2y,y2p */
    ...
    /** Enable or disable X/Y scale aspect locking for the view.
     ...
     set an unlocked aspect, but any other action changing the view scale will
    ...
    /** Checks if we are repainting.
     ...
     @retval FALSE
     */
    inline bool IsRepainting () const { return m_repainting; }

    /** Set view to fit global bounding box of all plot layers and refresh
     display with UpdateAll(). Scale and position will be set to show all
     attached mpLayers. The X/Y scale aspect lock is taken into account.
     */
    void Fit ();

    /** Set view to fit a given bounding box and refresh display with
     UpdateAll(). The X/Y scale aspect lock is taken into account. If provided,
     the parameters printSizeX and printSizeY are taken as the DC size, and the
     pixel scales are computed accordingly. Also, in this case the passed
     borders are not saved as the "desired borders", since this use will be
     invoked only when printing.
     */
    void Fit (const mpFloatRect &rect, wxCoord *printSizeX = NULL,
              wxCoord *printSizeY = NULL);

    /** Zoom into current view and refresh display
     * @param centerPoint The point (pixel coordinates) that will stay in the
     * same position on the screen after the zoom (by default, the center of
     * the mpWindow).
     */
    ...
    /** Zoom view fitting given coordinates to the window (p0 and p1 do not
    ...
    /** Counts the number of plot layers, including axes: this is to count only
     the layers which have a bounding box.
     \return The number of profiles plotted.
     */
    unsigned int CountLayers ();

    /** Counts the number of plot layers, whether or not they have a bounding
     box.
     \return The number of layers in the mpWindow. */
    unsigned int CountAllLayers () { return (unsigned int)m_layers.size (); }

    /** Counts the number of plot layers: this is to count only the layers
     which have a plot.
     \return The number of profiles plotted.
     */
    ...
    /// Get the 'desired' user-coordinate bounding box for the currently
    /// displayed view (set by Fit or Zoom operations).
    /// @sa Fit, Zoom
    mpFloatRect GetDesiredBoundingBox () const { return m_desired; }

    /** Returns the left-border layer coordinate that the user wants the
     * mpWindow to show (it may be not exactly the actual shown coordinate in
     * the case of locked aspect ratio).
     * @sa Fit, Zoom
     */
    double GetDesiredXmin () const { return m_desired.Xmin; }

    /** Returns the right-border layer coordinate that the user wants the
     * mpWindow to show (it may be not exactly the actual shown coordinate in
     * the case of locked aspect ratio).
     * @sa Fit, Zoom
     */
    double GetDesiredXmax () const { return m_desired.Xmax; }
     ...
     * @sa Fit, Zoom
     */
    double GetDesiredYmin () const { return m_desired.Ymin; }

    /** Returns the top layer-border coordinate that the user wants the
     * mpWindow to show (it may be not exactly the actual shown coordinate in
     * the case of locked aspect ratio).
     * @sa Fit, Zoom
     */
    double GetDesiredYmax () const { return m_desired.Ymax; }

    /** Returns the bounding box coordinates
     @param bbox Pointer to a 6-element double array where to store bounding
     box coordinates. */
    void GetBoundingBox (double *bbox) const;
    mpFloatRect *GetBoundingBox (void) { return &m_bound; }
    ...
    /** Draw the window on a wxBitmap, then save it to a file.
     ...
     @param type image type to be saved: see wxImage output file types for
     flags
     ...
     */
    wxBitmap *BitmapScreenshot (wxSize imageSize = wxDefaultSize,
                                bool fit = false);

    /**
     * Send the screen shot to the Clipboard
     */
    void ClipboardScreenshot (wxSize imageSize = wxDefaultSize,
                              bool fit = false);

    /**
     * Load a data file like a csv file
     * Data must be formatted in 2 columns X value and Y value separated by
     * space or ; or tab character
     */
    bool LoadFile (const wxString &filename);

    /** This value sets the zoom steps whenever the user clicks "Zoom in/out"
     * or performs zoom with the mouse wheel. It must be a number above unity.
     * This number is used for zoom in, and its inverse for zoom out. Set
     * to 1.5 by default. */
    static double m_zoomIncrementalFactor;

    /** Set window margins, creating a blank area where some kinds of layers
     cannot draw.
     * This is useful for example to draw axes outside the area where the plots
     are drawn.
    ...
    int GetMarginTop () const { return m_margin.top; }
    ...
    int GetMarginRight () const { return m_margin.right; }
    ...
    int GetMarginBottom () const { return m_margin.bottom; }
    ...
    int GetMarginLeft () const { return m_margin.left; }

    /** Get the width of the plot. */
    int GetPlotWidth () const { return m_plotWidth; }

    /** Get the height of the plot. */
    int GetPlotHeight () const { return m_plotHeight; }
    ...
    /** Check if a given point is inside the area of a mpInfoLayer and
     eventually returns its pointer.
    ...
    /** Set Color theme. Provide colours to set a new colour theme.
     ...
     @param drawColour The colour used to draw all elements in foreground, axes
     excluded
    ...
    const wxColour &GetAxesColour () const { return m_axColour; }

    const wxColour &GetbgColour () const { return m_bgColour; }

    void SetbgColour (const wxColour &colour) { m_bgColour = colour; }
    ...
    void UnSetOnDeleteLayer () { m_OnDeleteLayer = NULL; }

    /** On user mouse action event
     * Allows the user to perform certain actions before normal event
     * processing. The user has the possibility to interrupt or continue the
     * normal processing of the event.
     */
    void
    SetOnUserMouseAction (const mpOnUserMouseAction &userMouseEventHandler)
    {
      m_OnUserMouseAction = userMouseEventHandler;
    }

    /** Remove the 'user mouse action event' callback */
    void UnSetOnUserMouseAction () { m_OnUserMouseAction = NULL; }

    /**
     * Log axis control.
     * It is an axis property but as we need to control the bound and the
     * scale, it is easiest to declare this property here
     */
    bool IsLogXaxis () const { return m_LogXaxis; }

    bool IsLogYaxis () const { return m_LogYaxis; }

    void SetLogXaxis (bool log) { m_LogXaxis = log; }

    void SetLogYaxis (bool log) { m_LogYaxis = log; }

    bool GetMagnetize () const { return m_magnetize; }

    void SetMagnetize (bool mag) { m_magnetize = mag; }
    ...
    bool Y2AxisExist (void) const { return m_countY2Axis > 0; }

  protected:
    virtual void OnPaint (
      wxPaintEvent &event); //!< Paint handler, will plot all attached layers
    virtual void OnSize (
      wxSizeEvent &event); //!< Size handler, will update scroll bar sizes
    ...
    virtual void
    OnToggleGrids (wxCommandEvent &event); //!< Context menu handler
    virtual void
    OnToggleCoords (wxCommandEvent &event); //!< Context menu handler
    virtual void
    OnScreenShot (wxCommandEvent &event); //!< Context menu handler
    virtual void
    OnFullScreen (wxCommandEvent &event); //!< Context menu handler
    virtual void
    OnConfiguration (wxCommandEvent &event);         //!< Context menu handler
    ...
    virtual void
    OnLockAspect (wxCommandEvent &event);             //!< Context menu handler
    virtual void OnMouseHelp (wxCommandEvent &event); //!< Context menu handler
    virtual void OnMouseLeftDown (
      wxMouseEvent &event); //!< Mouse left click (for rect zoom)
    virtual void OnMouseRightDown (
      wxMouseEvent
        &event); //!< Mouse handler, for detecting when the user drags with the
                 //!< right button or just "clicks" for the menu
    virtual void OnMouseMove (
      wxMouseEvent &event); //!< Mouse handler for mouse motion (for pan)
    virtual void OnMouseLeftRelease (
      wxMouseEvent &event); //!< Mouse left click (for rect zoom)
    virtual void
    OnMouseWheel (wxMouseEvent &event); //!< Mouse handler for the wheel
    virtual void OnMouseLeftDoubleClick (
      wxMouseEvent &event); //!< Mouse handler for the left button double click
    virtual void OnMouseRightDoubleClick (
      wxMouseEvent
        &event); //!< Mouse handler for the right button double click
    virtual void OnMouseLeave (
      wxMouseEvent &event); //!< Mouse handler for mouse motion (for pan)
    bool CheckUserMouseAction (
      wxMouseEvent &event); //!< Check if a user mouse action is required
    virtual void OnScrollThumbTrack (
    ...
    virtual void
    OnScrollPageDown (wxScrollWinEvent &event); //!< Scroll page down
    virtual void OnScrollLineUp (wxScrollWinEvent &event); //!< Scroll line up
    virtual void
    OnScrollLineDown (wxScrollWinEvent &event);         //!< Scroll line down
    virtual void OnScrollTop (wxScrollWinEvent &event); //!< Scroll to top
    virtual void
    OnScrollBottom (wxScrollWinEvent &event); //!< Scroll to bottom
    ...
    /** Set bounding box 'm_bound' to contain all visible plots of this
     * mpWindow.
    ...
    mpScaleX
      *m_XAxis; //!< Pointer to the optional X axis layer of this mpWindow
    mpScaleY
      *m_YAxis; //!< Pointer to the optional Y axis layer of this mpWindow
    mpScaleY
      *m_Y2Axis; //!< Pointer to the optional Y2 axis layer of this mpWindow
    ...
    mpFloatRect m_bound; //!< Global layer bounding box in user coordinates.
    ...
    int m_scrX;     //!< Current view's X dimension in DC units, including all
                    //!< scales, margins
     ...
     *  May be different from the real borders (layer coordinates) only if lock
     * aspect ratio is true.
     */
    mpFloatRect m_desired; //!< ToDo: Set in Zoom, Fit(mpFloatRect), used in
                           //!< scrolling and?

    mpRect m_margin;      //!< Margin around the plot. Default 50
    wxCoord m_plotWidth;  //!< Width of the plot = m_scrX - (m_margin.left +
                          //!< m_margin.right)
    wxCoord m_plotHeight; //!< Height of the plot = m_scrY - (m_margin.top +
                          //!< m_margin.bottom)

    mpRect m_plotBoundaries;       //!< The full size of the plot. Calculated
    mpRect m_plotBoundariesMargin; //!< The size of the plot with the margins.
                                   //!< Calculated
    ...
    mpOnUserMouseAction m_OnUserMouseAction
      = NULL; //!< Event when we have a mouse click

    /// To be notified of displayed bounds changes (after user zoom etc),
    /// override this callback in your derived class and look at the new value
    /// of m_desired. Useful for keeping multiple plots in sync when user
    /// zooms.
  ...
  private:
    ...
    bool m_initialDesiredBoundsRecorded
      = false; //!< Has m_lastDesiredReportedBounds been set?
    mpFloatRect
      m_lastDesiredReportedBounds; //!< for use in DesiredBoundsHaveChanged
                                   //!< reporting in Fit()
  ...
  /** Plot layer implementing a text string.
   ...
   coordinates for the location are not required, and the text stays on the
   plot reguardless of the other layers location and scaling factors. second :
   center of a margin
   */
  ...
  public:
     ...
     */
    mpTitle (const wxString &name) : mpText (name, mpMarginTopCenter)
    {
      m_subtype = mptTitle;
      SetPen (*wxWHITE_PEN);
  ...
  public:
     ...
     */
    void SetFactor (int factor) { stretch_factor = factor; }
  ...
  //-----------------------------------------------------------------------------
  /** This virtual class represents objects that can be moved to an arbitrary
   * 2D location+rotation. The current transformation is set through
   * SetCoordinateBase. To ease the implementation of descendent classes,
   * mpMovableObject will be in charge of Bounding Box computation and layer
   * rendering, assuming that the object updates its shape in m_shape_xs &
   * m_shape_ys. We derive from mpFunction to have symbol and continuity
   * properties
   */
  class WXDLLIMPEXP_MATHPLOT mpMovableObject : public mpFunction
  {
  public:
    /** Default constructor (sets mpMovableObject location and rotation to
     * (0,0,0))
     */
    mpMovableObject ()
      : m_reference_x (0), m_reference_y (0), m_reference_phi (0),
        m_shape_xs (0), m_shape_ys (0)
    {
      assert (m_type == mpLAYER_PLOT); // m_type is already set to mpLAYER_PLOT
                                       // in default-arg mpFunction ctor:
                                       // m_type = mpLAYER_PLOT;
    ...
    /** Set the coordinate transformation (phi in radians, 0 means no
     * rotation).
     */
  ...
  protected:
     ...
     */
    void TranslatePoint (double x, double y, double &out_x,
                         double &out_y) const;

    /** This contains the object points, in local coordinates (to be
     * transformed by the current transformation).
     */
    std::vector<double> m_shape_xs, m_shape_ys;

    /** The buffer for the translated & rotated points (to avoid recomputing
     * them with each mpWindow refresh).
     *
     */
     ...
     * (m_shape_xs/ys), or when the transformation changes. This method updates
     * the buffers m_trans_shape_xs/ys, and the precomputed bounding box.
     */
    void ShapeUpdated ();
  ...
  public:
     ...
     * (m_continuous=true).
     */
    mpCovarianceEllipse (double cov_00 = 1, double cov_11 = 1,
                         double cov_01 = 0, double quantiles = 2,
                         int segments = 32, const wxString &layerName = _T(""))
      : mpMovableObject (), m_cov_00 (cov_00), m_cov_11 (cov_11),
        m_cov_01 (cov_01), m_quantiles (quantiles), m_segments (segments)
    {
      m_continuous = true;
      m_name = layerName;
      RecalculateShape ();
    ...
    /** Set how many "quantiles" to draw, that is, the confidence interval of
     * the ellipse (see above).
     */
  ...
  protected:
    /** The elements of the matrix (only 3 since cov(0,1)=cov(1,0) in any
    ...
    /** Called to update the m_shape_xs, m_shape_ys vectors, whenever a
     * parameter changes.
     */
   ...
   * non-closed polygons by passing the appropriate parameters to "setPoints".
   * To draw a point-cloud, call "SetContinuity(false)".
   */
  ...
  /** A layer that allows you to have a bitmap image printed in the mpWindow.
   ...
   You can retrieve the image from the layer at anytime you want with
   getBitmapCopy()
   */
  ...
  public:
    ...
    /** Change the bitmap associated with the layer (to update the screen,
     * refresh the mpWindow).
