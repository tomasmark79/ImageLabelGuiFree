...
int mainIsOmited (int argc, const char *argv[])
{
  ...
  try
  {
    ...
    if (!result.count ("omit"))
    {
      ...
      std::unique_ptr<library::WxWidgetsAdapter> lib
        = std::make_unique<library::WxWidgetsAdapter> (assetsPath);
    }
    else
    {
